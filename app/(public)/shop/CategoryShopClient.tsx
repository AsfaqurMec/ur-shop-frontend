...
export function CategoryShopClient({
  ...
  useEffect(() => {
    ...
    nextPage.current = 2;
  }, [initialProducts, total, categorySlug, search, minPrice, maxPrice, onSale, sort]);
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm font-medium text-muted-foreground">
        Showing <span className="tabular-nums text-foreground">{products.length}</span> of{' '}
        <span className="tabular-nums text-foreground">{total}</span> products
      </p>
      <ProductGrid products={products} onAddToCart={handleAddToCart} addingProductId={addingProductId} />
      <div ref={sentinelRef} className="flex min-h-12 justify-center" aria-live="polite">
        {loadingMore ? (
          <span
          ...
          <span className="text-sm text-muted-foreground">You have reached the end.</span>
        ) : null}
      </div>
    </div>
  );
