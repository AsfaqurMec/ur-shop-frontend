...
export function ShopClient({ initialProducts, total, search, minPrice, maxPrice }: ShopClientProps) {
  ...
  useEffect(() => {
    ...
    nextPage.current = 2;
  }, [initialProducts, total, search, minPrice, maxPrice]);

  const loadMore = async () => {
    if (loadingMore || !hasMore) return;
    setLoadingMore(true);
    try {
      const result = await fetchProducts({ page: nextPage.current, limit: 8, search, min_price: minPrice, max_price: maxPrice, is_active: true });
      setProducts((current) => {
